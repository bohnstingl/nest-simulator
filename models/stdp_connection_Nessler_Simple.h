...
#ifndef STDP_CONNECTION_NESSLER_SIMPLE_H
#define STDP_CONNECTION_NESSLER_SIMPLE_H
...
class STDPConnectionNesslerSimple : public Connection< targetidentifierT >
{
...
public:
   ...
   */
  STDPConnectionNesslerSimple( const STDPConnectionNesslerSimple& );
...
private:
  double
  facilitate_( double w )
  {    
    double newWeight = w + eta_ * (c_ * std::exp(- (w - baseline_)) - 1.);
    return newWeight > Wmax_ ? Wmax_ : newWeight;
  }

  double
  depress_( double w )
  {
    double newWeight = w + eta_ * (-1.0);
    return newWeight < 0.0 ? 0.0 : newWeight; 
  }
...
template < typename targetidentifierT >
inline void
STDPConnectionNesslerSimple< targetidentifierT >::send( Event& e,
  thread t,
  double t_lastspike,
...
template < typename targetidentifierT >
STDPConnectionNesslerSimple< targetidentifierT >::STDPConnectionNesslerSimple()
  : ConnectionBase()
  , weight_( 1.0 )
  , Wmax_( 200. )
  , c_( 1.0 )
...
template < typename targetidentifierT >
STDPConnectionNesslerSimple< targetidentifierT >::STDPConnectionNesslerSimple(
  const STDPConnectionNesslerSimple< targetidentifierT >& rhs )
  : ConnectionBase( rhs )
  , weight_( rhs.weight_ )
  , Wmax_( rhs.Wmax_ )
  , c_( rhs.c_ )
...
template < typename targetidentifierT >
void
STDPConnectionNesslerSimple< targetidentifierT >::get_status( DictionaryDatum& d ) const
{
...
template < typename targetidentifierT >
void
STDPConnectionNesslerSimple< targetidentifierT >::set_status( const DictionaryDatum& d,
  ConnectorModel& cm )
{
