...
#ifndef PP_PSC_DELTA_NESSLER_H
#define PP_PSC_DELTA_NESSLER_H
 ...
 */
class pp_psc_delta_Nessler : public Archiving_Node
{

public:
  pp_psc_delta_Nessler();
  pp_psc_delta_Nessler( const pp_psc_delta_Nessler& );
...
private:
  ...
  friend class RecordablesMap< pp_psc_delta_Nessler >;
  friend class UniversalDataLogger< pp_psc_delta_Nessler >;
  ...
  struct Parameters_
  {
    ...
    /** Does the model uses excitability updates after a spike */
    bool is_excitable_;
  ...
  struct Buffers_
  {
    Buffers_( pp_psc_delta_Nessler& );
    Buffers_( const Buffers_&, pp_psc_delta_Nessler& );
    ...
    UniversalDataLogger< pp_psc_delta_Nessler > logger_;
  };
  ...
  static RecordablesMap< pp_psc_delta_Nessler > recordablesMap_;
};

inline port
pp_psc_delta_Nessler::send_test_event( Node& target,
  rport receptor_type,
  synindex,
  bool )
{
...
inline port
pp_psc_delta_Nessler::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
pp_psc_delta_Nessler::handles_test_event( CurrentEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline port
pp_psc_delta_Nessler::handles_test_event( DataLoggingRequest& dlr, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline void
pp_psc_delta_Nessler::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d, P_ );
...
inline void
pp_psc_delta_Nessler::set_status( const DictionaryDatum& d )
{
...
#endif /* #ifndef PP_PSC_DELTA_NESSLER_H */
